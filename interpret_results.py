...
def determine_optimal_section(mass, harmonic, deflection):
    ...
    normalized_mass = 1. - (mass - min_mass) / (max_mass - min_mass)
    normalized_harmonic = (harmonic - min_harmonic) / (max_harmonic - min_harmonic)
    normalized_deflection = 1. - (deflection - min_deflection) / (max_deflection - min_deflection)
    
    # assign equal weighting to mass and harmonic
    mass_weighting = 1/3
    harmonic_weighting = 1/3
    deflection_weighting = 1/3
    score =  mass_weighting * normalized_mass + harmonic_weighting * normalized_harmonic + deflection_weighting * normalized_deflection

    high_score_index = np.argmax(score)
    return high_score_index

def plot_mass_vs_deflection(mass_uls_passed, mass_uls_failed, deflection_uls_passed, deflection_uls_failed,
                                optimal_mass, optimal_deflection, optimal_sections, sheet_name, out_path):

    plt.figure(figsize=(10,6))
    plt.scatter(mass_uls_failed, deflection_uls_failed*1000, label='ULS Failed', color='red', s=10)
    plt.scatter(mass_uls_passed, deflection_uls_passed*1000, label='ULS Passed', color='green', s=10)
    plt.scatter(optimal_mass, optimal_deflection*1000, label='Optimal Section Combination', color='cyan', s=20)
    ...
    plt.minorticks_on()
    plt.legend()
    plt.annotate(optimal_sections, xy=(optimal_mass, optimal_deflection*1000), xytext=(optimal_mass + 400, optimal_deflection*1000 + 2),
                 arrowprops=dict(facecolor='cyan', arrowstyle='->'), fontsize=9, bbox=dict(boxstyle="round,pad=0.3", edgecolor='cyan',
                                                                                           facecolor='white', alpha=0.8))

    plot_save(plt, out_path, sheet_name, 'mass vs deflection')

def plot_mass_vs_harmonic(mass_uls_passed, mass_uls_failed, harmonic_uls_passed, harmonic_uls_failed,
                                optimal_mass, optimal_harmonic, optimal_sections, sheet_name, out_path):
    
    plt.figure(figsize=(10,6))
    plt.scatter(mass_uls_failed, harmonic_uls_failed, label='ULS Failed', color='red', s=10)
    plt.scatter(mass_uls_passed, harmonic_uls_passed, label='ULS Passed', color='green', s=10)
    plt.scatter(optimal_mass, optimal_harmonic, label='Optimal Section Combination', color='cyan', s=20)
    plt.ylabel('Occupied resonating harmonic')
    plt.xlabel('Mass of module (kg)')
    plt.title(f'Module mass vs occupied resonating harmonic for "{sheet_name}" sections')
    plt.grid(True)
    plt.minorticks_on()
    plt.legend()
    plt.annotate(optimal_sections, xy=(optimal_mass, optimal_harmonic), xytext=(optimal_mass + 400, optimal_harmonic - 0.7),
                 arrowprops=dict(facecolor='cyan', arrowstyle='->'), fontsize=9, bbox=dict(boxstyle="round,pad=0.3", edgecolor='cyan',
                                                                                           facecolor='white', alpha=0.8))
...
def interpret_results(file_path, sheets, folderpath):
    ...
    for index, df in enumerate(dfs):
        ...
        high_score_index = determine_optimal_section(mass_uls_passed, harmonic_uls_passed, deflection_uls_passed)
        ...
        optimal_sections = "Top: " + optimal_top_chord + ", Bottom: " + optimal_bottom_chord + ", Web: " + optimal_web
        # with line breaks for plotting 
        optimal_sections_spaced = "Top: " + optimal_top_chord + "\nBottom: " + optimal_bottom_chord + "\nWeb: " + optimal_web

        plot_mass_vs_deflection(mass_uls_passed, mass_uls_failed, deflection_uls_passed, deflection_uls_failed,
                                optimal_mass, optimal_deflection, optimal_sections_spaced, sheets[index], out_path)
        plot_mass_vs_harmonic(mass_uls_passed, mass_uls_failed, harmonic_uls_passed, harmonic_uls_failed,
                                optimal_mass, optimal_harmonic, optimal_sections_spaced, sheets[index], out_path)
